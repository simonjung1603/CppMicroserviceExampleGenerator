...
int main(int argc, char* argv[])
{
  ...
  try
  {
    ...
    // bundle expects a @{service_implementation_name} service in its activator Start()
    ...
    auto iter = std::find_if(bundles.begin(), bundles.end(), [](Bundle& b) {
      return b.GetSymbolicName() == "@{service_implementation_dir}";
    });
    if (iter != bundles.end())
    {
      iter->Start();
...
CPPMICROSERVICES_IMPORT_BUNDLE(@{service_implementation_dir})
CPPMICROSERVICES_IMPORT_BUNDLE(@{service_consumer_dir})
#endif
